...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('slideshow', () => ({
        ...
        total: 0,
        showHelp: false,
        showOverview: false, // Overview Mode State
        lastMove: Date.now(),
        isIdle: false,
        ...
        init() {
            ...
            window.addEventListener('keydown', (e) => {
                ...
                if (e.key === 'Escape') {
                    ...
                    else if (this.showOverview) this.showOverview = false;
                    else this.showOverview = true; // Toggle overview on Esc if nothing else open
                }
            });
                ...
                evtSource.onmessage = (event) => {
                    try {
                        ...
                        if (data.type === 'control') {
                            ...
                            if (data.action === 'goto') this.goto(data.index); // Use the new goto method
                        }
                        if (data.type === 'vote') {
